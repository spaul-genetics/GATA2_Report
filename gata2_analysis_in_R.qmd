...
#data$mutation.Class[data$mutation.Class=='c term'] = 'C term'
...
#mod.data$mutation.Class = factor(mutation.Class, levels = c("Enhancer","C term","Missense 2nd ZF","Null","Truncation"))%>%
  

#mod.data = mod.data%>%filter(ageonset!=0)
#mod.data = mod.data%>%mutate(mutation.Class = relevel(factor(mutation.Class), ref = 'Enhancer'))
#mod.data = mod.data%>%mutate(Missense.group = relevel(factor(Missense.group), ref = 'R398'))
```
